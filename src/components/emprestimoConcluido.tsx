...
import { Chave, Sala, Solicitante, Responsavel, Emprestimo } from "../pages/emprestimos";
...
export function EmprestimosConcluidos({salas, chaves, solicitantes, responsaveis, pesquisa}: EmprestimosConcluidosProps) {
  ...
  const [filtroDataEmprestimoRetiradaConcluidos, setFiltroDataEmprestimoRetiradaConcluidos] = useState<DateRange | undefined>();
  ...
  const getNomeSala = (idSala: number) => salas.find((sala) => sala.id === idSala)?.nome || "";
  const getNomeChave = (idChave: number) => chaves.find((chave) => chave.id === idChave)?.nome || "";
  const getNomeResponsavel = (idResponsavel: number) => responsaveis.find((responsavel) => responsavel.id === idResponsavel)?.nome || "";
  const getNomeSolicitante = (idSolicitante: number) => solicitantes.find((solicitante) => solicitante.id === idSolicitante)?.nome || "";
  ...
  const emprestimosFiltradosConcluidos = emprestimosConcluidos
    ...
    .filter((emprestimos) => {
      ...
      const dataEmprestimo = converterDataBRparaDate(emprestimos.dataRetirada);

      return (
        !filtroDataDevolucao?.from ||
        !filtroDataDevolucao?.to ||
        (dataEmprestimo >= filtroDataDevolucao.from &&
          dataEmprestimo <= filtroDataDevolucao.to)
      );
    })
    .filter((emprestimos) => {
      if (
      ...
      const dataEmprestimo = emprestimos.dataDevolucao ? converterDataBRparaDate(emprestimos.dataDevolucao) : null;
            ...
            .map((emprestimo, index) => (
              <tr key={index}>
                <td className="p-2 text-sm text-[#646999] font-semibold border-2 border-solid border-[#B8BCE0] break-words w-[13%]">
                  {salas.find((sala) => sala.id === emprestimo.sala)?.nome ||
                    "Sala não encontrada"}
                </td>
                <td className="p-2 text-sm text-[#646999] font-semibold border-2 border-solid border-[#B8BCE0] break-words w-[13%]">
                  {chaves.find((chave) => chave.id === emprestimo.chave)
